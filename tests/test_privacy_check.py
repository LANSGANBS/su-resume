...
class TextScannerTests(unittest.TestCase):
    def test_repository_placeholders_are_allowed(self):
        sample = "\n".join(
            (
                "you@example.com",
                r"(+86) 1XX\,XXXX\,XXXX",
                "https://example.com/code/yourname",
    ...
    def test_project_pdf_author_allowlist_is_exact(self):
        self.assertIn(
            "tex-pdf-author",
            rules_for("pdf" + "author={A Person, su-resume contributors}"),
        )
    ...
    def test_all_zero_numeric_phone_is_not_treated_as_reserved(self):
        phone = "138" + "0000" + "0000"
        self.assertIn("phone-number", rules_for(phone))

    def test_github_support_allowlist_is_exact(self):
        self.assertEqual(rules_for("support@" + "github.com"), set())
        self.assertIn(
            "non-example-email",
            rules_for("other@" + "github.com"),
        )
...
class HistoryScannerTests(unittest.TestCase):
    ...
    def named_commit_environment(self, name: str, email: str):
        environment = os.environ.copy()
        environment.update(
            {
    ...
    def test_history_allows_exact_dependabot_support_identity_only(self):
        with tempfile.TemporaryDirectory() as directory:
            repository = Path(directory)
            self.run_git(repository, "init", "-q")
            fixture = repository / "safe.txt"
            ...
            self.run_git(
                ...
                env=self.named_commit_environment(
                    "dependabot[bot]",
                    "support@github.com",
                ),
            )
