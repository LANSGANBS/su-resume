...
def run_text_tool(
    command: list[str], *, reject_markers: tuple[str, ...] = ()
) -> str:
    ...
    failures = [marker for marker in reject_markers if marker in diagnostics]
    if failures:
        raise RuntimeError(
            "tool reported unusable font/CMap output: " + ", ".join(failures)
        )
...
def extract_pdf_text(path: Path) -> str:
    ...
    body = run_text_tool(
        ...
        reject_markers=PDF_TEXT_FAILURE_MARKERS,
    )
    if not "".join(body.split()):
        raise RuntimeError("pdftotext extracted no visible text")
