...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="content.tex",
        help=(
            "Content TeX file relative to the repository, or an absolute "
            "path in the private workspace."
        ),
    )
    parser.add_argument(
        "--theme",
...
def resolve_content_file(repo_root: Path, value: str) -> Path:
    ...
    if not candidate.is_file():
        raise ValueError(f"content file does not exist: {candidate}")
    return candidate
...
def validate_source_layout(main_file: Path, content_file: Path) -> list[str]:
    ...
    if "\\ResumeContentFile" not in main_text:
        errors.append("main file does not expose \\ResumeContentFile")
    elif not re.search(
        r"\\input\s*\{\s*\\ResumeContentFile\s*\}", main_text
    ):
        errors.append("main file does not input \\ResumeContentFile")
...
def tex_entry_expression(
    main_relative: str, theme: str, content_file: Path | None = None
) -> str:
    unsupported = ('"', "{", "}", "%", "#", "\n", "\r")
    if any(character in main_relative for character in unsupported):
        raise ValueError("main TeX path contains a character unsupported by XeLaTeX")
    normalized = main_relative.replace("\\", "/")
    content_definition = ""
    if content_file is not None:
        content_path = content_file.resolve().as_posix()
        if any(character in content_path for character in unsupported):
            raise ValueError(
                "content TeX path contains a character unsupported by XeLaTeX"
            )
        content_definition = (
            rf"\def\ResumeContentFile{{\detokenize{{{content_path}}}}}"
        )
    return (
        rf"\def\ResumeTheme{{{theme}}}"
        f"{content_definition}"
        rf'\input{{"{normalized}"}}'
    )
...
def run_xelatex(
    ...
    expression = tex_entry_expression(main_relative, theme, content_file)
    command = [
        xelatex,
        "-no-shell-escape",
...
def renderer_output_usable(
    renderer: str, returncode: int, diagnostics: str
) -> bool:
    if returncode != 0:
        return False
    ...
    return not any(marker in normalized for marker in PDF_RENDER_FAILURE_MARKERS)
...
def render_pdf(
        ...
        commands.append(
            (
                ...
                [
                    ...
                    f"-sOutputFile={(output_dir / 'page-%d.png').as_posix()}",
                    str(pdf_path),
                ],
            )
        )
    ...
    if not commands:
        raise RuntimeError(
            "could not render PDF; install pdftoppm, Ghostscript, mutool, or pdftocairo"
        )
    ...
    for renderer, command in commands:
        ...
        usable = renderer_output_usable(
            renderer, completed.returncode, diagnostics
        )
        attempts.append(
            {
                "renderer": renderer,
                "returncode": completed.returncode,
                "font_diagnostics_rejected": (
                    completed.returncode == 0 and not usable
                ),
            }
        )
        if not usable:
        ...
        try:
            ...
            continue
        return images, renderer, attempts

    summary = ", ".join(
        f"{attempt['renderer']}={attempt['returncode']}"
        + ("/font-diagnostics" if attempt["font_diagnostics_rejected"] else "")
        for attempt in attempts
    )
    raise RuntimeError(f"every PDF renderer failed or was unusable ({summary})")
...
def validate_theme(
    *,
    ...
    try:
        images, renderer, render_attempts = render_pdf(
            pdf_path, output_dir, render_dpi
        )
        rendered = []
        for image in images:
        ...
        result["render_attempts"] = render_attempts
...
def main() -> int:
    ...
    try:
        ...
        content_file = resolve_content_file(repo_root, args.content)
    ...
    try:
        tex_entry_expression(main_relative, themes[0], content_file)
    except ValueError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 2
    content_sha256_before = file_digest(content_file)
    results = [
        validate_theme(
            xelatex=xelatex,
            repo_root=repo_root,
    ...
    content_sha256_after = file_digest(content_file)
    content_preserved = content_sha256_after == content_sha256_before
    if not content_preserved:
        for result in results:
            result["status"] = "failed"
            result["errors"].append("content file changed during validation")
    ...
    manifest = {
        ...
        "main": str(main_file),
        "main_sha256": file_digest(main_file),
        "content": str(content_file),
        "content_sha256": content_sha256_after,
        "content_preserved": content_preserved,
        "theme_file": str(theme_file),
        "theme_sha256": file_digest(theme_file),
        "expected_pages": args.expected_pages,
        "render_dpi": args.render_dpi,
        "max_overfull_pt": args.max_overfull_pt,
