...
ASCII_PLACEHOLDER_RE = re.compile(r"\b(?:TODO|TBD|FIXME)\b", re.IGNORECASE)
CHINESE_PLACEHOLDER_RE = re.compile(
    r"^\s*(?:[【\[(（]\s*)?待(?:确认|补充)(?:\s*[】\])）])?(?:\s*[:：]|$)"
)
...
def contains_placeholder(value: Any) -> bool:
    if not isinstance(value, str):
        return False
    return bool(
        ASCII_PLACEHOLDER_RE.search(value)
        or CHINESE_PLACEHOLDER_RE.search(value)
    )
...
def validate_ledger(
    ...
    for index, fact in enumerate(facts):
        ...
        if status not in VALID_FACT_STATUSES:
            ...
            continue
        # Category is deliberately open vocabulary. It helps retrieval and
        # grouping but must never become a resume-domain enum.
        if not nonempty_string(fact.get("category")):
            errors.append(
                f"{fact_id}.category must be a non-empty open-vocabulary string"
            )
        ...
        if status == "usable":
            ...
            if not nonempty_string(evidence) or contains_placeholder(evidence):
                errors.append(f"{fact_id}.evidence must be concrete and contain no TODO")
            if not nonempty_string(public_text) or contains_placeholder(public_text):
                errors.append(f"{fact_id}.public_text must be final and contain no TODO")
        elif status == "needs-confirmation":
            if nonempty_string(public_text) and not contains_placeholder(public_text):
                errors.append(
    ...
    for index, question in enumerate(questions):
        ...
        if not nonempty_string(fact_id):
            errors.append(f"{question_id}.fact_id must reference a fact")
        elif fact_id not in facts_by_id:
