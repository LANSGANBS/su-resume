...
EXACT_GENERIC_PDF_AUTHORS = {
    "su-resume contributors",
}
...
def is_example_email(address: str) -> bool:
    ...
    return (
        ...
        or address.casefold() == "support@github.com"
...
def scan_text(location: str, text: str) -> List[Finding]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        for match in TEX_PDF_AUTHOR_RE.finditer(line):
            ...
            if (
                author
                and author not in EXACT_GENERIC_PDF_AUTHORS
                and not any(
...
def safe_pdf_author(value: str) -> bool:
    ...
    return (
        not normalized
        or normalized in EXACT_GENERIC_PDF_AUTHORS
        or any(marker in normalized for marker in GENERIC_PDF_AUTHORS)
    )
...
def public_commit_identity(name: str, address: str) -> bool:
    if address.casefold() == "support@github.com":
        return " ".join(name.casefold().split()) == "dependabot[bot]"
    if public_commit_email(address):
        return True
    return False
...
def scan_history(
    ...
    for commit in commits:
        ...
        if not public_commit_identity(author_name, author_email):
            findings.append(
                Finding(
                    label,
                    None,
        ...
        if not public_commit_identity(committer_name, committer_email):
            findings.append(
                Finding(
                    label,
                    None,
